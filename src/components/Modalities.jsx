      ...
      <div className="container mx-auto px-4 ">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8 ">
          {modalities.map((modality, index) => (
            <motion.div
              key={modality.name}
              className={`relative rounded-lg overflow-hidden shadow-lg group ${modality.isNew ? "cursor-pointer border-2 border-accent" : "border border-tertiary"} bg-secondary transition-transform duration-300`}
            ...
            >
              ...
              />
              <div className={`p-4  cursor-pointer ${modality.isNew ? "bg-accent text-primary" : "bg-secondary"}`}>
                <h3 className="text-lg sm:text-xl font-bold mb-1 cursor-pointer">{modality.name}</h3>
                <p className={`text-sm cursor-pointer ${modality.isNew ? "text-primary-700" : "text-white-50"}`}>
                  {modality.description}
                </p>
                {modality.isNew && (
                  <span className="absolute top-4 right-4 bg-[#FFA500]  border-[#FFA500]  text-accent text-xs font-bold px-3 py-1 rounded-full">
                    NOVA!
                  </span>
                )}
              </div>
